...
public class ScanService
{
    ...
    /**
     * 앱이 꺼지거나 새 버전으로 바뀔 때. 도는 스캔에 취소를 걸어 다음 확인 지점에서 멈추게
     * 한다. 이게 없어도 잠금은 풀린다 — 스토어가 사라져 다음 접근에서 던지고 finally 가
     * 놓는다(실측 36번에서 관찰) — 하지만 그건 예외로 멈추는 것이고, 이건 멈추라고 해서
     * 멈추는 것이다. 어느 쪽이든 그 실행은 행을 남기지 못한다(스토어가 이미 없다).
     */
    @PreDestroy
